...
class GraphLLM(nn.Module):
    ...
    def forecast(self, x_enc, x_feature):
        ...
        llm_prompt_st = torch.cat([llm_prompt_out, llm_st_out], dim=1)
        llm_align_out = self.attn_layers_align(x_res, llm_prompt_st)
        llm_fusion = self.out_fusion(llm_align_out, llm_align_out)
